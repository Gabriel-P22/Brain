...
- `.claude/agents/python-tutor.md` — dedicated subagent for Python, but NOT beginner teaching (Python is the user's strongest language already): gap-filling on modern features, code review, and serving as the comparison-anchor side when a Go topic needs contrast. Follows the `*-MODULES.md`/`README.md`/`exercise/` flow only if `contexts/PYTHON-MODULES.md` exists; otherwise free-form Q&A.
- `.claude/agents/ia-tutor.md` — dedicated subagent for the `ia/` material (LLMs, RAG, agents, MCP, harness, spec-driven development, eval/security/observability), same `*-MODULES.md`/`README.md`/`exercise/` conventions as `go-tutor` against `contexts/IA-MODULES.md`, but with no `config/reference/` (IA isn't a language with its own idiomatic SOLID/DDD example) and no fixed schedule (no `*_STUDY_PLAN.md` counterpart to sync). Usable standalone for focused IA Q&A without pulling in unrelated session context.
